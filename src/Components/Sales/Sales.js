...
const Sales = () => {
    ...
    useEffect(() => {
        ...
        const payPointz = paymentMethods.reduce((obj, method) => {
            obj[method.name] = ''
            return obj
        }, {})
...
const AddProduct = ({
    ...
    const [wrh, setWrh] = useState(isProductView ? Object.keys(salesEntries)[0] : (wrhs.find((wr) => { return !wr.purchase })?.name || 'default'))
