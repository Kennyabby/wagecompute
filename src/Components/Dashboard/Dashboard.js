...
const Dashboard = ()=>{
    ...
    useEffect(()=>{
        ...
        if (dashList.includes(path)){
            ...
            else if (index === 7 && (companyRecord?.status === 'admin' || companyRecord?.permissions.includes('sales'))){
                setView(<Sales/>)
            }
            else if (index === 8 && (companyRecord?.status === 'admin' || companyRecord?.permissions.includes('inventory'))){
                setView(<Inventory/>)
            }
            else if (index === 9 && (companyRecord?.status === 'admin' || companyRecord?.permissions.includes('accommodations'))){
                setView(<Accommodation/>)            
            }
            else if (index === 10 && (companyRecord?.status === 'admin' || companyRecord?.permissions.includes('purchase'))){
                setView(<Purchase/>)
            }
            else if (index === 11 && (companyRecord?.status === 'admin' || companyRecord?.permissions.includes('expenses'))){
                setView(<Expenses/>)
            }
            else if (index === 12 && (companyRecord?.status === 'admin' || companyRecord?.permissions.includes('reports'))){
                setView(<Reports/>)
            }
            else if (index === 13 && (companyRecord?.status === 'admin' || companyRecord?.permissions.includes('settings'))){
                setView(<Settings/>)
            }else{
                setView('')
            }
