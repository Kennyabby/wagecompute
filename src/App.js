    ...
    try {
          ...
          prop: [
            {
              $match: {
                $expr: {
                  $lt: [{ $toString: "$postingStamp" }, formattedStartDate],
                },
              },
            },
            {
      ...
      if (openingStockResp.record) {
        openingStockResp.record.forEach((item) => {
          ...
          stockMap[productId][location].openingQuantity =
            item.openingQuantity || 0;
          stockMap[productId][location].openingCost =
            products.find((p) => p.i_d === productId)?.salesPrice &&
            item.purchasedQty
              ? Number(
                  (
                    (item.purchaseCost / item.purchasedQty) *
          ...
          stockMap[productId][location].closingSalesValue =
            stockMap[productId][location].closingQty *
            (products.find((p) => p.i_d === productId)?.salesPrice || 0);
          stockMap[productId][location].averageCost =
            item.openingQuantity !== 0
              ? Number((closingCost / item.openingQuantity).toFixed(2))
              : 0;
        });
      }
      ...
      if (transactionsResp.record) {
        transactionsResp.record.forEach((item) => {
          ...
          // Net adjustments
          locationData.netAdjustmentQty =
            (item.positiveAdjustmentQty || 0) -
            (item.negativeAdjustmentQty || 0);
          locationData.netAdjustmentCost =
            (item.positiveAdjustmentCost || 0) -
            (item.negativeAdjustmentCost || 0);

          // Calculate closing quantities
          locationData.closingQty =
            (locationData.openingQuantity || 0) +
          ...
          locationData.closingCost = products.find((p) => p.i_d === productId)
            ?.salesPrice
            ? locationData.closingQty * locationData.averageCost
            : 0;
          locationData.closingSalesValue =
            locationData.closingQty *
            (products.find((p) => p.i_d === productId)?.salesPrice || 0);
        });
      }
      ...
      const enrichedProducts = products.map((product) => {
        ...
        const totals = Object.values(locationData).reduce(
          (acc, loc) => ({
            openingQuantity:
              (acc.openingQuantity || 0) + (loc.openingQuantity || 0),
            ...
            costOfGoodsSold:
              (acc.costOfGoodsSold || 0) + (loc.costOfGoodsSold || 0),
            transferInQty: (acc.transferInQty || 0) + (loc.transferInQty || 0),
            transferOutQty:
              (acc.transferOutQty || 0) + (loc.transferOutQty || 0),

            // Adjustment fields
            positiveAdjustmentQty:
              (acc.positiveAdjustmentQty || 0) +
              (loc.positiveAdjustmentQty || 0),
            positiveAdjustmentCost:
              (acc.positiveAdjustmentCost || 0) +
              (loc.positiveAdjustmentCost || 0),
            negativeAdjustmentQty:
              (acc.negativeAdjustmentQty || 0) +
              (loc.negativeAdjustmentQty || 0),
            negativeAdjustmentCost:
              (acc.negativeAdjustmentCost || 0) +
              (loc.negativeAdjustmentCost || 0),
            netAdjustmentQty:
              (acc.netAdjustmentQty || 0) + (loc.netAdjustmentQty || 0),
            netAdjustmentCost:
              (acc.netAdjustmentCost || 0) + (loc.netAdjustmentCost || 0),

            // Closing values
            closingQty: (acc.closingQty || 0) + (loc.closingQty || 0),
            averageCost:
              (acc.averageCost || 0) +
              (loc.closingQty ? loc.closingQty / loc.closingCost : 0),
            closingCost: (acc.closingCost || 0) + (loc.closingCost || 0),
            closingSalesValue:
              (acc.closingSalesValue || 0) + (loc.closingSalesValue || 0),
          }),
        ...
        const netAdjustmentQty =
          (totals.positiveAdjustmentQty || 0) -
          (totals.negativeAdjustmentQty || 0);
        const netAdjustmentCost =
          (totals.positiveAdjustmentCost || 0) -
          (totals.negativeAdjustmentCost || 0);
